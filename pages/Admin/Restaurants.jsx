...
function Restaurants() {
  ...
  const fetchOwners = async () => {
    try {
      ...
      console.error(err);
      toast.error("Failed to fetch restaurant owners");
    }
  };

  // Find owner for a specific restaurant
  const getRestaurantOwner = (restaurant) => {
    if (!restaurant.owner) {
      return null;
    }

    // If restaurant.owner is already an object
    if (typeof restaurant.owner === "object") {
      return restaurant.owner;
    }

    // If restaurant.owner is just an ID
    return owners.find(
      (owner) => owner._id === restaurant.owner
    );
  };
